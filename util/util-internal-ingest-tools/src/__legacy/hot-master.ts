...
// This is the `hot.ts` from `master` (commit 1f7fdbef) preserved verbatim
// so that `hot.parity.test.ts` can compare old behavior against the
// current implementation. The `HotProcessor` here is exported as
// `LegacyHotProcessor` to avoid collision.
...
export class LegacyHotProcessor<B> {
