...
import {ConnectionQuery, CountQuery, EntityByPkQuery, ListQuery, Query} from '../../sql/query'
...
export class SchemaBuilder {
        ...
        for (let name in this.model) {
            ...
            switch(item.kind) {
                case "entity":
                    ...
                    this.installEntityByPk(name, query, subscription)
    ...
    private installEntityByPk(entityName: string, query: GqlFieldMap, subscription: GqlFieldMap): void {
        let entity = getEntity(this.model, entityName)
        let pk = entity.pk ?? ['id']
        let singular = this.normalizeEntityName(entityName).singular

        // `queryName` renames the single-entity lookup. With a composite key
        // ByPks is that lookup, so the override lands there instead of on ById.
        let byPksName = (pk.length > 1 && entity.queryName) || `${singular}ByPks`
        this.installPkLookup(entityName, pk, byPksName, query, subscription)

        if (pk.length == 1) {
            // `id` alone already is the whole key, so the long-standing ById
            // query keeps working unchanged, alongside ByPks.
            let byIdName = entity.queryName || `${singular}ById`
            assert(
                byIdName !== byPksName,
                `queryName '${byIdName}' of ${entityName} collides with its generated ByPks query`
            )
            this.installPkLookup(entityName, pk, byIdName, query, subscription)
        }
    }

    private installPkLookup(
        entityName: string,
        pk: string[],
        queryName: string,
        query: GqlFieldMap,
        subscription: GqlFieldMap
    ): void {
        let model = this.model
        ...
        for (let field of pk) {
            let prop = entity.properties[field]
            assert(prop != null, `${entityName}.${field} is a part of its pk, but is not a property`)
            // Model validation restricts pk columns to scalars and enums, both
            // of which are valid GraphQL input types.
            assert(
