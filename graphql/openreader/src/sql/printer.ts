...
import { isSQLiteFamily, DbType } from '../context';
...
export class EntitySqlPrinter {
    ...
    private sub(entityName: string, args?: SqlArguments, fields?: FieldRequest[]): EntitySqlPrinter {
        return new EntitySqlPrinter(this.model, this.dbType, entityName, this.params, args, fields, this.aliases);
    }
        ...
        for (let f of fields) {
            switch (f.kind) {
                ...
                case 'list-lookup': {
                    ...
                    let exp = isSQLiteFamily(this.dbType) ?
                        `(SELECT json_group_array(json(row)) FROM (${sub.printAsJsonRows()}) AS rows)`:
                        `(SELECT jsonb_agg(row) FROM (${sub.printAsJsonRows()}) AS rows)`;
    ...
    private scalarBinaryCondition(sqlOp: string, cursor: Cursor, where: {
        field: string,
        value: unknown
    }, exps: string[]): void {
        let f = cursor.native(where.field);
    ...
    private refBinaryCondition(sqlOp: string, cursor: Cursor, where: {
        field: string,
        value: unknown
    }, exps: string[]): void {
    ...
    private isSQLiteFamily() {
        return isSQLiteFamily(this.dbType)
    }
    ...
    private ident(name: string): string {
        return escapeIdentifier(this.dbType, name);
    }
    ...
    printColumnList(options?: { withAliases?: boolean }): string {
        assert(this.hasColumns(), 'no columns selected');
    ...
    printColumnListAsJsonArray(): string {
        if (isSQLiteFamily(this.dbType)) return `json_array(${this.printColumnList()})`;
