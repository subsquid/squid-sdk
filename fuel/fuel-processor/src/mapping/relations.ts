...
//     block.receipts.sort(instructionCompare)
...
//         let ins = block.receipts[i]
//         let tx = txs[ins.transactionIndex]
//         if (tx) {
//             ins.transaction = tx
//             tx.receipts.push(ins)
//         }
//         for (let j = i + 1; j < block.receipts.length; j++) {
//             let next = block.receipts[j]
//             if (isInner(ins, next)) {
//                 ins.inner.push(next)
...
//         for (let i = bisectInstructions(block.receipts, log); i < block.receipts.length; i++) {
//             let ins = block.receipts[i]
//             if (isInner(ins, log)) {
//                 ins.logs.push(log)
