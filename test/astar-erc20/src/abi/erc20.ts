...
export class MulticallContract {
  ...
  'decimals()' = {
    ...
    tryCall: (args: string[]): Promise<Result<number>[]> => this.tryCall('decimals()', args.map((arg) => [arg, []]))
  }
  ...
  'balanceOf(address)' = {
    ...
    tryCall: (args: [string, [_owner: string]][]): Promise<Result<ethers.BigNumber>[]> => this.tryCall('balanceOf(address)', args)
  }
  ...
  'allowance(address,address)' = {
    ...
    tryCall: (args: [string, [_owner: string, _spender: string]][]): Promise<Result<ethers.BigNumber>[]> => this.tryCall('allowance(address,address)', args)
  }
