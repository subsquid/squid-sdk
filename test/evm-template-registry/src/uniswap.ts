...
import {FACTORY_UNI, TEMPLATE_UNI_PAIR} from './constants'
import type {Log} from './source'

async function handlePairCreated(log: Log, blockNumber: number): Promise<void> {
    const template = useTemplate(TEMPLATE_UNI_PAIR)
    const store = useStore()

    if (log.address.toLowerCase() !== FACTORY_UNI) return
...
export async function handleUni(log: Log, blockNumber: number): Promise<void> {
    switch (log.topics[0]) {
