import {AbiParameter} from "./decodeAbiParameters";
...
export function numberToHex(
  ...
  if ((maxValue && value > maxValue) || value < minValue) {
    throw new Error(`Number "${value}" is not in safe ${signed ? 'signed' : 'unsigned'} integer range [${minValue}, ${maxValue}]`);
  }
...
const concat = (hexes: Hex[]): Hex => `0x${hexes.map((h) => h.slice(2)).join('')}`
...
function encodeAddress(value: Hex): PreparedParam {
  if (!value.match(/^0x[0-9a-fA-F]{40}$/)) throw new Error(`Invalid address: ${value}`);
...
function encodeArray<const TParam extends AbiParameter>(
  ...
  if (!dynamic && value.length !== length)
    throw new Error(`Invalid ${param.type}[${length}] array length: ${value.length}`);
...
function encodeString(value: string): PreparedParam {
  ...
  return {
    dynamic: true,
    encoded: concat([
      padHex(numberToHex(encoded.length)),
      ...parts,
    ]),
  }
}
