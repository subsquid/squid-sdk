...
function safeSignedRangeCodec<T extends number | bigint | boolean | string>(
  ...
  const overflowErr = `${typeName} overflows`;
  const underflowErr = `${typeName} underflows`;
  return {
    isDynamic: false,
    encode(sink: Sink, val: T) {
      const bigVal = BigInt(val);
      assert(bigVal >= min, overflowErr);
      assert(bigVal <= max, underflowErr);
      sink.i256(bigVal);
    },
    decode(src: Src): T {
      const val = src.i256();
      assert(val >= min, overflowErr);
      assert(val <= max, underflowErr);
...
function safeUnsignedRangeCodec<T extends number | bigint | boolean | string>(
  ...
  const negativeErr = `Negative value provided for ${typeName}`;
  const overflowErr = `${typeName} overflows`;
  return {
    isDynamic: false,
    encode(sink: Sink, val: T) {
      const bigVal = BigInt(val);
      assert(bigVal >= 0, negativeErr);
      assert(bigVal <= max, overflowErr);
      sink.u256(bigVal);
    },
    decode(src: Src): T {
      const val = src.u256();
      assert(val <= max, overflowErr);
      return converter(val);
    },
  };
}

const uintCodec = (bits: number) =>
  safeUnsignedRangeCodec((1n << BigInt(bits)) - 1n, `uint${bits}`, Number);
const intCodec = (bits: number) =>
  safeSignedRangeCodec(
    -(1n << BigInt(bits - 1)),
    (1n << BigInt(bits - 1)) - 1n,
    `int${bits}`,
    Number
...
const bigUintCodec = (bits: number) =>
  safeUnsignedRangeCodec((1n << BigInt(bits)) - 1n, `uint${bits}`, BigInt);
