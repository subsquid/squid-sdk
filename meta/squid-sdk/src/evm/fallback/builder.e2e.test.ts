...
import {type EvmFallbackSourceConfig, EvmFallbackDataSourceBuilder} from './builder'
...
function fallback(primary: EvmFallbackSourceConfig) {
    return new EvmFallbackDataSourceBuilder()
        .setDownstreamSources([primary, {type: 'rpc', url: RPC_URL, capacity: 5}])
...
describe.skipIf(!ENABLED)('EVM fallback — live', () => {
    it('streams a range through the primary (Portal)', async () => {
        let fb = fallback({type: 'portal', url: PORTAL_URL})
